...
def apply_scenario_overrides(
    ...
    *,
    ...
    shares so the Mission-Inputs panel can override δ_ops and the schema-v9
    payload requirement (``payload_mass_kg`` / ``payload_power_w``) without
    each route re-implementing the merge. ``None`` fields fall through to
    the scenario's calibrated/class-typical default. Returns the input
    scenario unchanged when no override is supplied (no needless copy).
    """
